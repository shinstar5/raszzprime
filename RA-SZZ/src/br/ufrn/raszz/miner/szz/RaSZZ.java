...
public class RaSZZ extends Miner {
	...
	private void buildAnnotationGraph() throws Exception {
		try {
			...
			boolean isTest = (Boolean) this.getParameters().get("isTest");	
			String threadsProperty = this.getProperty("threads","/backhoe.properties");
			int threads = 0;
			if (threadsProperty != null) {
				try {
						threads = Integer.valueOf(threadsProperty);
				} catch (NumberFormatException e) {
						// handle error, maybe set a default value for threads
						threads = 1;  // example default value
				}
		} else {
				// handle error, maybe set a default value for threads
				threads = 1;  // example default value
